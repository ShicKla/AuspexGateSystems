...
function userInput(x, y)
  ...
  while true do
    ...
    if event == "key_down" then
      if arg2 ~= 28 and arg2 ~= 14 and arg1 ~= 0 then -- Not Enter, Backspace, or Other
        inputString = inputString..unicode.char(arg1)
        gpu.fill(x, y, unicode.len(inputString), 1, " ")
        gpu.set(x, y, inputString)
      elseif arg2 == 14 then -- Backspace Key
        inputString = unicode.wtrunc(inputString, unicode.len(inputString))
        gpu.fill(x, y, 20, 1, " ")
        gpu.set(x, y, inputString)
      elseif arg2 == 28 then -- Enter Key
        break
      end
      term.setCursor(x+unicode.len(inputString), y)
    end
  end
...
function gateEntriesWindow.display()
  ...
  if #self.entryStrings > self.range.height then gpu.set(1, 41, "╚══════════════╡Scroll with PgUp/PgDn╞╝") end
...
function glyphListWindow.touch(x,y)
  local self = glyphListWindow
  if not dialingMode and not dhdDialing and not dialerAdrEntryMode and not self.locked then
...
function glyphListWindow.insertGlyph(glyph)
  if gateType == "UN" then glyph = string.gsub(glyph, "G", "Glyph ") end
...
function dhdAddressEntry()
  ...
  while dhdAdrEntryMode do
    ...
    if sg.getGateStatus() == "failing" then dhdAdrEntryMode = false end -- (Backup Check of Failing Event) Implies the BRB was pressed
  end
  if allGood then
    addressBuffer = {}
    local glyph = ""
...
function dialerAddressEntry()
  ...
  while sg.getGateStatus() == "unstable" do os.sleep(0.01) end
  sg.disengageGate()
  dialerAdrEntryMode = false
  if allGood then
...
function editGateEntry(index)
  ...
  for i,v in ipairs(gateEntry.gateAddress.UN) do
    gpu.set(61+(8-math.floor(unicode.len(v)/2)), 9+i, v)
  end
...
failEvent = event.listen("stargate_failed", function(evname, address, caller, reason)
  if not dhdAdrEntryMode then
    ...
    while sg.getGateStatus() == "failing" do os.sleep(0.01) end
    dialingMode = false

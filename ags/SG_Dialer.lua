...
local RS_Settings = {GateCloseSide="north", ChevronEngagedSide="south"}
...
function abortDialing()
  ...
  if sg.abortDialing ~= nil and true then 
    directAbortDialing()
  else
    -- buttons.abortDialingButton:disable(true)
    -- AbortingDialing = true
...
local EventListeners = {
  ...
  stargate_close = event.listen("stargate_close", function(_, _, caller, reason)
    ...
    if HasRedstone then
      ...
      -- event.timer(1, function() redstone.setOutput(sides[RS_Settings.GateCloseSide], 15) end)
    end
  end),
